...
def photoscan_alignphotos(images, reference_eo, sequence):
    ...
    for i in range(len(chunk.cameras)):
        ...
        chunk.cameras[i].reference.rotation = (float(reference_eo[6 * i + 5]), float(reference_eo[6 * i + 4]) + 90, float(reference_eo[6 * i + 3]))

    chunk.camera_location_accuracy = PhotoScan.Vector([10, 10, 10])
    chunk.camera_rotation_accuracy = PhotoScan.Vector([10, 10, 10])
