...
if __name__ == '__main__':
    ...
    for root0, dirs0, files0 in os.walk('/internalCompany/PM2019007_nifs/DKC/yeosu_stacks_test'):
        dirs0.sort()
        for dir0 in dirs0:
            dstPath = root0 + '/' + dir0 + '_orthophotos/'
            if dir0[-12:] == 'orthophotos/':
                continue
            for root, dirs, files in os.walk(root0 + '/' + dir0):
                files.sort()
                print(files)
                for file in files:
                    filename = os.path.splitext(file)[0]
                    extension = os.path.splitext(file)[1]
                    file_path = root0 + '/' + dir0 + '/' + file
